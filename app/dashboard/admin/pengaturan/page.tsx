...
export default function MenuPengaturan() {
  ...
  // --- KAMUS TRANSLASI LOKAL HALAMAN (DITAMBAH ALAY & JAKSEL AGAR TIDAK ERROR) ---
  ...
  const tp = (key: string) => {
    // Tambahkan fallback bahasa 'id' jika bahasa yang dipilih belum ada di kamus lokal
    ...
    <div className={`max-w-5xl mx-auto space-y-10 animate-in fade-in duration-700 ${cur.text}`}>
      ...
      <div className={`flex justify-between items-end border-b pb-8 ${cur.border}`}>
        ...
        <button 
          onClick={handleSave}
          disabled={saving}
          className={`${saving ? 'bg-gray-400' : 'bg-blue-600'} text-white px-10 py-4 rounded-[2rem] font-black text-[10px] uppercase shadow-xl active:scale-95 transition-all`}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className={`${cur.card} border ${cur.border} p-10 rounded-[3.5rem] space-y-6 shadow-sm`}>
            <h3 className="text-xs font-black uppercase text-blue-600 tracking-widest border-b border-blue-600/10 pb-2">{tp('branding_header')}</h3>
            <div className="space-y-4">
                ...
                <div className="space-y-1">
                    <label className={`text-[9px] font-black uppercase ${cur.textMuted} ml-2`}>{tp('color_lbl')}</label>
                    <div className="flex gap-4 items-center">
                        <input type="color" className="w-12 h-12 rounded-xl cursor-pointer bg-transparent border-none" value={settings.primary_color} onChange={(e) => setSettings({...settings, primary_color: e.target.value})} />
                        <p className={`font-mono text-xs font-bold opacity-60 uppercase ${cur.text}`}>{settings.primary_color}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className={`${cur.card} border ${cur.border} p-10 rounded-[3.5rem] space-y-6 shadow-sm`}>
            ...
            <div className="grid grid-cols-1 gap-3">
                <button onClick={handleBackup} className={`w-full flex justify-between items-center p-5 bg-green-600/5 hover:bg-green-600/10 border border-green-600/10 rounded-2xl transition-all ${cur.text}`}>
                    <span className="text-[10px] font-black uppercase">{tp('backup_lbl')}</span>
                    <span className="text-xl">📥</span>
                </button>
                <label className={`w-full flex justify-between items-center p-5 bg-blue-600/5 hover:bg-blue-600/10 border border-blue-600/10 rounded-2xl transition-all cursor-pointer ${cur.text}`}>
                    <span className="text-[10px] font-black uppercase">{tp('restore_lbl')}</span>
                    <span className="text-xl">📤</span>
                    <input type="file" accept=".json" className="hidden" onChange={handleRestore} />
                </label>
                <button onClick={handleFactoryReset} className={`w-full flex justify-between items-center p-5 bg-red-600/5 hover:bg-red-600/10 border border-red-600/10 rounded-2xl transition-all ${cur.text}`}>
        ...
        <div className={`${cur.card} border ${cur.border} p-10 rounded-[3.5rem] space-y-6 shadow-sm`}>
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                    ...
                    <input className={`w-full ${cur.input} border ${cur.border} px-6 py-4 rounded-2xl font-bold focus:outline-none ${cur.text}`} value={settings.academic_year} onChange={(e) => setSettings({...settings, academic_year: e.target.value})} />
                </div>
                <div className="space-y-1">
                    <label className={`text-[9px] font-black uppercase ${cur.textMuted} ml-2`}>{tp('semester_lbl')}</label>
                    <select className={`w-full ${cur.input} border ${cur.border} px-6 py-4 rounded-2xl font-bold focus:outline-none ${cur.text}`} value={settings.semester} onChange={(e) => setSettings({...settings, semester: e.target.value})}>
