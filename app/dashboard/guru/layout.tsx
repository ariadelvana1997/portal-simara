...
const IconKokurikuler = () => <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"></path></svg>;
...
const IconKids = () => (<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M10 5a2 2 0 1 0 4 0a2 2 0 1 0 -4 0" /><circle cx="12" cy="13" r="8" /><path d="M12 9v4" /><path d="M9 14s1 1 3 1 3-1 3-1" /></svg>);
...
export default function GuruLayout({ children }: { children: React.ReactNode }) {
  ...
  // --- STATE KOORDINATOR KOKURIKULER ---
  const [isCoordinator, setIsCoordinator] = useState(false);
  const [isKokuOpen, setIsKokuOpen] = useState(false);
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (ghostUser) {
          ...
          // FIX: Cek koordinator di tabel 'kokurikuler_groups' milik kamu
          const { data: kokuData } = await supabase.from('kokurikuler_groups').select('id').eq('teacher_id', parsedGhost.id).limit(1);
          if (kokuData && kokuData.length > 0) setIsCoordinator(true);
        ...
        // FIX: Cek apakah ID Guru ini terdaftar di tabel 'kokurikuler_groups'
        const { data: kokuRes } = await supabase.from('kokurikuler_groups').select('id').eq('teacher_id', user.id).limit(1);
        if (kokuRes && kokuRes.length > 0) setIsCoordinator(true);
  ...
  return (
    <ThemeContext.Provider value={{ cur, mode, setMode, setIsLocked, profile, appConfig, setAppConfig, t }}>
      <div className={`min-h-screen flex transition-colors duration-700 ease-in-out ${cur.bg} ${cur.text} font-sans`}>
        ...
        {/* ... (Autopilot & Lock tetap sama) ... */}
        {profile?.is_autopilot && (
            <div className="fixed bottom-8 right-8 z-[10000] animate-bounce-slow">
                <button onClick={handleStopAutopilot} className="group flex items-center gap-3 bg-red-600 text-white pl-4 pr-6 py-3.5 rounded-full font-black text-[10px] uppercase tracking-[0.2em] shadow-2xl shadow-red-600/40 hover:bg-red-700 transition-all active:scale-95">
          ...
          <div className="fixed inset-0 z-[9999] bg-black flex flex-col items-center justify-center p-6 backdrop-blur-md animate-in fade-in duration-700">
              <div className="w-full max-sm text-center space-y-8 text-white">
                <div className="space-y-3">
                   <div className="w-20 h-20 bg-blue-600 rounded-[2.5rem] mx-auto flex items-center justify-center shadow-2xl rotate-12"><svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect><path d="M7 11V7a5 5 0 0 1 10 0v4"></path></svg></div>
        ...
        <aside className={`fixed inset-y-0 left-0 z-[70] border-r transition-[width,transform] duration-500 md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0 w-72' : '-translate-x-full'} ${isCollapsed ? 'md:w-20' : 'md:w-72'} ${cur.sidebar} ${cur.border} flex flex-col shadow-2xl md:shadow-none`}>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-2">
            ...
            {/* --- DROPDOWN PENILAIAN KOKURIKULER (HANYA KOORDINATOR) --- */}
            {isCoordinator && (
              <div className="space-y-1">
                <button onClick={() => setIsKokuOpen(!isKokuOpen)} className={`flex items-center w-full transition-all duration-300 group ${pathname.includes('/guru/kokurikuler') ? 'text-white shadow-lg' : `hover:translate-x-1 active:scale-95`} ${isCollapsed && !isSidebarOpen ? 'w-12 h-12 justify-center mx-auto' : 'gap-3 p-3'} ${cur.radius}`} style={{ backgroundColor: pathname.includes('/guru/kokurikuler') ? activeTheme.primary : 'transparent', color: pathname.includes('/guru/kokurikuler') ? 'white' : 'inherit' }} >
                  <span className={`transition-transform duration-300 ${pathname.includes('/guru/kokurikuler') ? 'scale-110' : 'group-hover:scale-110 opacity-70'}`}><IconKokurikuler /></span>
                  {(!isCollapsed || isSidebarOpen) && (
                    <div className="flex items-center justify-between flex-1"><span className="font-bold text-sm tracking-tight truncate">Penilaian Kokurikuler</span><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" className={`transition-transform duration-300 ${isKokuOpen ? 'rotate-180' : ''}`}><path d="M6 9l6 6 6-6"/></svg></div>
                  )}
                </button>
                {isKokuOpen && (!isCollapsed || isSidebarOpen) && (
                  <div className="ml-4 pl-4 border-l-2 border-gray-100 dark:border-gray-800 space-y-1 py-2 animate-in slide-in-from-top-2 duration-300">
                    <Link href="/dashboard/guru/kokurikuler/input" className={`block py-2 text-xs font-bold transition-all ${pathname === '/dashboard/guru/kokurikuler/input' ? 'text-blue-600 font-black' : 'opacity-50 hover:opacity-100'}`}>Input Nilai Kokurikuler</Link>
                    <Link href="/dashboard/guru/kokurikuler/import" className={`block py-2 text-xs font-bold transition-all ${pathname === '/dashboard/guru/kokurikuler/import' ? 'text-blue-600 font-black' : 'opacity-50 hover:opacity-100'}`}>Import Nilai Kokurikuler</Link>
                    <Link href="/dashboard/guru/kokurikuler/deskripsi" className={`block py-2 text-xs font-bold transition-all ${pathname === '/dashboard/guru/kokurikuler/deskripsi' ? 'text-blue-600 font-black' : 'opacity-50 hover:opacity-100'}`}>Deskripsi Kokurikuler</Link>
                  </div>
                )}
              </div>
            )}
        ...
        <div className="flex-1 flex flex-col min-w-0 transition-all duration-500">
           <header className={`h-16 flex items-center justify-between px-4 md:px-8 border-b ${cur.border} ${cur.header} sticky top-0 z-[55] backdrop-blur-md`}>
            <div className="flex items-center gap-4">
              <button onClick={() => { if (window.innerWidth < 768) { setSidebarOpen(true); } else { setIsCollapsed(!isCollapsed); } }} className={`p-2 ${cur.hover} ${cur.radius} active:scale-75 transition-all group`} ><svg width="22" height="22" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M4 6h16M4 12h10m-10 6h16" /></svg></button>
            ...
            <div className="flex items-center gap-4">
               <div className={`flex items-center ${cur.bg} p-1 ${cur.radius} border ${cur.border} gap-0.5 shadow-sm`}>
                  {['light', 'dark', 'read'].map((m: any) => (<button key={m} onClick={() => setMode(m)} className={`px-3 py-1.5 ${cur.radius} text-[10px] font-black uppercase transition-all duration-300 ${mode === m ? 'text-white shadow-lg scale-105' : 'opacity-40 hover:opacity-100'}`} style={{ backgroundColor: mode === m ? activeTheme.primary : 'transparent' }}>{m[0]}</button>))}
                  <div className={`w-[1px] h-4 ${cur.border} mx-1 opacity-50`}></div>
                  <button onClick={() => alert("🍭 Fitur Mode Anak sedang dalam tahap pengembangan!")} title="Mode Anak" className={`px-3 py-1.5 ${cur.radius} text-[10px] font-black transition-all bg-pink-500/10 text-pink-600 hover:bg-pink-500 hover:text-white active:scale-90`} ><div className="flex items-center gap-1.5"><IconKids /><span className="hidden lg:inline">MODE ANAK</span></div></button>
                  <button onClick={() => setIsLocked(true)} className={`px-3 py-1.5 ${cur.radius} text-[10px] font-black transition-all bg-red-500 text-white shadow-lg active:rotate-12`}>LOCK</button>
               </div>
               <div className={`w-9 h-9 ${cur.radius} flex items-center justify-center text-white text-xs font-black shadow-lg`} style={{ backgroundColor: activeTheme.primary }}>{profile?.full_name?.charAt(0) || 'G'}</div>
            </div>
          </header>
          <main className="p-4 md:p-8 overflow-y-auto bg-inherit"><div className="max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-2 duration-700">{children}</div></main>
        </div>
      </div>
